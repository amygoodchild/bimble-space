...
function drawBackground(){
  // Drawing background
  if (ps.chosenBackground == 0){
    ps.blendMode(ps.DIFFERENCE);
    ps.bgOpacityToUse = ps.backgroundOpacity;
  }
  else if (ps.chosenBackground == 1){
    ps.blendMode(ps.ADD);
    ps.bgOpacityToUse = ps.backgroundOpacity/15;
  }
  else{
    ps.blendMode(ps.BLEND);
    ps.bgOpacityToUse = ps.backgroundOpacity/3.5;
  }
  ps.fill(ps.red(ps.backgrounds[ps.chosenBackground]),ps.green(ps.backgrounds[ps.chosenBackground]),ps.blue(ps.backgrounds[ps.chosenBackground]),ps.bgOpacityToUse);
  ps.noStroke();
